...
GUARDRAIL_NAME="US Cached Models Only"
...
GUARDRAILS=$(curl -s "${API_BASE}/guardrails" \
...
if [[ -n "$GUARDRAIL_ID" ]]; then
  ...
  RESPONSE=$(curl -s -X PUT "${API_BASE}/guardrails/${GUARDRAIL_ID}" \
    -H "Authorization: Bearer ${OPENROUTER_PROVISIONING_KEY}" \
  ...
  RESPONSE=$(curl -s -X POST "${API_BASE}/guardrails" \
    -H "Authorization: Bearer ${OPENROUTER_PROVISIONING_KEY}" \
    -H "Content-Type: application/json" \
    -d "$PAYLOAD")
